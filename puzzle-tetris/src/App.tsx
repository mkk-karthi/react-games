...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-ocean-50">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div
          className="absolute top-1/3 left-2 sm:left-[28.5%] text-2xl animate-float"
        ...
        <div
          className="absolute bottom-[30%] sm:bottom-1/4 left-1/3 text-3xl animate-float"
          style={{ animationDelay: "1.2s" }}
        >
          🐙
        </div>
        <div
          className="absolute top-1/2 right-2 sm:right-[28.5%] text-3xl animate-float"
        ...
        {/* Bubbles */}
        <div
          className="absolute top-20 right-1/3 w-3 h-3 rounded-full bg-cyan-300/40 animate-bubble"
          style={{ animationDelay: "0s" }}
        ></div>
        <div
          className="absolute top-40 left-1/3 w-4 h-4 rounded-full bg-blue-300/40 animate-bubble"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
        ...
        ></div>
        <div
          className="absolute top-1/2 left-1/4 w-3 h-3 rounded-full bg-cyan-200/40 animate-bubble"
          style={{ animationDelay: "1.5s" }}
        ></div>
      </div>

      <main className="relative w-full grid sm:grid-cols-7 sm:grid-rows-5 gap-2 sm:gap-3 p-4 sm:h-screen z-10">
        {/* Header */}
        <div className="sm:col-span-2 sm:row-span-1 flex items-center justify-center">
          <h1 className="font-semibold uppercase tracking-[0.2em] my-2">
            <span className="text-2xl">🐬</span>
            <span className="text-3xl text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 via-blue-400 to-teal-300 align-sub mx-2">
              Puzzle Tetris
            </span>
            <span className="text-2xl">🌊</span>
          </h1>
        ...
        {/* Board */}
        <div className="sm:col-span-3 sm:row-span-5 sm:col-start-3 relative mx-auto w-full">
          <Board
            board={state.board}
            active={state.active}
            ghost={ghost}
            className="mx-auto max-w-[86%] my-3"
          />
          <BoardOverlay
            status={state.status}
        ...
        {/* Stat */}
        <div className="sm:col-span-2 sm:row-span-1 sm:col-start-6 hidden sm:block">
          <div className="flex justify-center items-center gap-3 sm:mt-3 py-3 h-full">
        ...
        <div className="sm:col-span-2 sm:row-span-2 sm:col-start-6 row-start-2">
          <Hud score={state.score} level={state.level} lines={state.lines} />
        </div>
        <div className="sm:col-span-2 sm:row-span-2 sm:col-start-6 sm:row-start-4">
          <NextQueue pieces={nextPieces} />
        </div>
        <div className="hidden sm:col-span-2 sm:row-span-4 sm:row-start-2 sm:grid gap-3">
          <ControlsCard />
        </div>
      </main>
    </div>
