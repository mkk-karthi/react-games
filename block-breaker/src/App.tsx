...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const updateBoardSize = () => {
      ...
      // On mobile, account for header and UI height
      const verticalPadding = isMobile ? (isSmallHeight ? 120 : 160) : 100;
      const horizontalPadding = isMobile ? 20 : 80;

      const maxWidth = Math.min(window.innerWidth - horizontalPadding, 900);
      const maxHeight = Math.min(window.innerHeight - verticalPadding, 800);
  ...
  const handleRestart = () => {
    if (engineRef.current) {
      if (gameState && gameState.gameStatus == "gameOver") engineRef.current.resetLevelProgress();
  ...
  return (
    <div
      className="flex items-center justify-center min-h-screen p-2 sm:p-4 font-cyber overflow-x-hidden flex-col lg:flex-row gap-2 sm:gap-4"
      style={{
        background: "linear-gradient(135deg, #0a0a0a 0%, #1a0a2e 50%, #0a0a0a 100%)",
      }}
    >
      <h1 className="text-4xl font-bold m-1 sm:m-2 text-glow bg-gradient-to-r from-neon-pink via-neon-purple to-neon-blue bg-clip-text text-transparent text-center lg:[writing-mode:sideways-lr] order-1">
        Block Breaker
      </h1>
      {/* Game UI */}
      {gameState ? (
      ...
      <div
        ref={containerRef}
        className="relative glass-effect rounded-2xl overflow-hidden shadow-2xl order-3 lg:order-2"
        style={{
